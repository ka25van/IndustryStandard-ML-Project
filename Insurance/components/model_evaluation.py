...
class ModelEvaluation:
    def __init__(self,model_eval_config:config_entity.ModelEvaluationConfig,
                ...
                try:
                    ...
                    self.model_resolver = ModelResolver()  #we need the predictor.py, as it has the new models
    ...
    def initiate_model_evaluation(self)->artifact_entity.ModelEvaluationArtifact:
        try:
            ...
            #Reading data from the above saved files path
            ...
            #Testing to compare new mode with old model
            #For that we need to read the data
            # take test data for testing test data 

            test_df = pd.read_csv(self.data_ingestion_artifact.test_file_path)
            target_df = test_df[TARGET_COLUMN]
            y_true = target_df  # y_true means true labels
            ...
            input_feature_name = list(transformer.feature_names_in_)  #we are calling the feature in from transformer
            for i in input_feature_name:       
                if test_df[i].dtypes =='object':
                    test_df[i]=target_encoder.fit_transform(test_df[i])  

            input_arr =transformer.transform(test_df[input_feature_name])  #transforming input_feature_name in test_df
            y_pred = model.predict(input_arr)  #means data predicted by model

            #Finding accuracy of old model; r2 score is needed for prediction
            print(f"Prediction using previous model: {y_pred[:5]}")
            previous_model_score = r2_score(y_true=y_true, y_pred=y_pred)
            logging.info(f"Accuracy using previous trained model: {previous_model_score}")
           
            # Accuracy for current trained model
            ...
            if current_model_score<=previous_model_score:  #Choosing between new and old model accuracies
                logging.info(f"Current trained model is not better than previous model")
                raise Exception("Current trained model is not better than previous model")
            else:
                logging.info(f"Current trained model is better than previous model")
...
#After this we need to this to be stored
#So we can do it in the cloud like AWS s3 bucket or DataBase or in the local system itself(like saved_model here)